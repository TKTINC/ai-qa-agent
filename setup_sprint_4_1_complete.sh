...
# AI QA Agent - Sprint 4.1 (COMPLETE VERSION)
    ...
    <script>
        class AgentChatInterface {
            ...
            connectWebSocket() {
                ...
                const wsUrl = `${protocol}//${window.location.host}/web/api/v1/agent/conversation/stream/${this.sessionId}`;
            ...
            async sendMessage() {
                ...
                try {
                    const response = await fetch('/web/api/v1/agent/conversation', {
                        method: 'POST',
                        headers: {
...
if ! grep -q "from src.web.routes.agent_interface import router as web_router" src/api/main.py; then
...
if ! grep -q "jinja2==3.1.3" requirements.txt; then
