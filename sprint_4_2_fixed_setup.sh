...
# AI QA Agent - Sprint 4.2 (FIXED VERSION)
...
if [ ! -f "src/web/components/agent_chat.py" ]; then
    ...
    echo "ℹ️  Note: If you haven't completed Sprint 4.1, we'll create placeholder components"
    
    # Create minimal prerequisites for Sprint 4.2 to work standalone
    mkdir -p src/web/components
    mkdir -p src/web/services
    
    echo "📄 Creating minimal agent chat component..."
    cat > src/web/components/agent_chat.py << 'EOF'
"""
Agent Chat Component (Placeholder for Sprint 4.2)
"""
...
EOF
    ...
    echo "📄 Creating minimal agent visualization service..."
    cat > src/web/services/agent_visualization.py << 'EOF'
"""
Agent Visualization Service (Enhanced for Sprint 4.2)
"""
...
class AgentVisualizationService:
    ...
    def _generate_sample_data(self):
        """Generate sample data for demonstration"""
        agents = ["test_architect", "code_reviewer", "performance_analyst", "security_specialist", "documentation_expert"]
        activity_types = ["reasoning", "collaboration", "response", "analysis"]
        learning_types = ["improvement", "adaptation", "knowledge", "pattern"]
        
        # Generate activities for last 24 hours
        for i in range(50):
        ...
        for i in range(20):
            self.learning_events.append(
                LearningEvent(
                    ...
                    description=f"Sample learning event {i}",
    ...
    async def get_system_intelligence_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "system_health_score": random.uniform(0.8, 0.95),
            "total_agents": 5,
            "active_sessions": random.randint(10, 50),
            "avg_response_time": random.uniform(0.2, 0.8)
        }
    
    async def get_live_activity_feed(self, limit: int = 10) -> List[Dict[str, Any]]:
        """Get recent activity feed"""
        recent_activities = sorted(self.agent_activities, key=lambda x: x.timestamp, reverse=True)[:limit]
...
# Install new dependencies for analytics and visualization (FIXED - removed chart-js)
...
echo "ℹ️  Note: Chart.js is loaded via CDN in HTML templates, not via pip"
...
EOF
    ...
    echo "⚠️ Warning: src/api/main.py not found. Please add analytics routes manually."
fi
...
# Sprint 4.2 - Analytics Dependencies (FIXED - removed chart-js)
plotly==5.17.0
pandas==2.1.4
numpy==1.24.4
bokeh==3.3.2
...
python3 -m pytest tests/unit/web/analytics/test_intelligence_dashboard.py -v || echo "ℹ️ Tests may require dependencies to be installed"
...
echo ""
echo "🔧 FIXED: Removed chart-js dependency (JavaScript library, not Python package)"
echo ""
